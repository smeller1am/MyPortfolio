...
const API_URL = "";
async function cartAdd(id, variation, quantity) {
...
async function cartDelete(id, variation) {
...
async function cartHasProduct(id, variation) {
...
async function compareAdd(product_id, category_id) {
...
async function compareDelete(product_id, category_id) {
  const res = await fetch(
...
async function sendTradeInForm(formData) {
  const res = await fetch(API_URL + "/wp-content/themes/app-room-sa/mail.php", {
    credentials: "include",
    method: "POST",
    body: formData
...
async function sendFilters(formData) {
...
async function sendSearchFilters(formData) {
...
function openDrawer(trigger, container) {
...
function closeDrawer(trigger, container) {
  let root = document.getElementsByTagName("html")[0];
  let main = document.querySelector(".main");
...
const trigger$1 = document.querySelector(".drawer-trigger");
const container$1 = document.querySelector(".drawer");
function onclick() {
  if (trigger$1 && container$1) {
    if (trigger$1.getAttribute("data-open") === "true") {
      closeDrawer(trigger$1, container$1);
    } else {
      openDrawer(trigger$1, container$1);
...
if (trigger$1) trigger$1.addEventListener("click", onclick);

const nav = document.querySelector(".navigation");
const navItems = document.querySelectorAll(".navigation__item[data-tab]");
const dropdownItems$1 = document.querySelectorAll(".dropdown__item[data-tab]");
const dropdown = document.querySelector(".dropdown");
function hoverNavItem(element) {
...
let timeLeave = 0;
function enterNav() {
  timeEnter = (/* @__PURE__ */ new Date()).valueOf();
  timeLeave = timeEnter;
}
function leaveNav() {
  timeLeave = (/* @__PURE__ */ new Date()).valueOf();
  setTimeout(() => {
    if (dropdown && timeEnter < timeLeave) {
      navItems.forEach(
        (item) => removeClass(item, "navigation__item--active")
      );
...
function setDataTab(element, id) {
...
const hoverMenuLink = (e) => {
  if (e.target instanceof Element) {
    ...
    if (menuId && tabId) {
      ...
      dropdownLinls.forEach(
        (link) => removeClass(link, "dropdown-column__link--active")
      );
...
menuLinks.forEach(
  (link) => link.addEventListener("pointerover", hoverMenuLink)
);

export { Accordion as A, Oe as O, sendOrder as a, cartDelete as b, cartAdd as c, sendTradeInForm as d, deleteTab as e, formatPrice as f, compareDelete as g, cartHasProduct as h, compareAdd as i, getDefaultExportFromCjs as j, sendSearchFilters as k, sendFilters as l, numberWithSpaces as n, sendForm as s };
