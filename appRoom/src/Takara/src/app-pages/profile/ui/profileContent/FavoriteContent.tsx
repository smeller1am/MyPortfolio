...
import { ProductModal } from '@/app-pages/category/ui/product-modal';
...
export const FavoriteContent: FC = () => {
  const { data: favorites, isLoading, isFetching } = useGetAllFavoritesQuery();
  ...
  const handleAddToBasketClick = (product: ProductModel) => {
    if (accessToken === '') {
      ...
      <div className="profile__target profile__target--active profile__target--opacity">
        ...
        <div className="likeInfo">
          {isLoading ? <div className="loaderWrapper">
            <Loader/></div>  : <div className="likeInfo__container">
                ...
                return (
                    <div key={favorite.id} className="likeInfo__grid-item">
                      <Image
                          ...
                          width={200}
                          height={200}
                          src={favorite.photoPath || '/img/defaultCard.png'}
                          alt=""
                          onClick={() => openProductModal(favorite)}
                      />
                      <div className="likeInfo__itemInfo">
                        <div className="likeInfo__itemInfo-title">{favorite.name}</div>
                        <div className="likeInfo__itemInfo-subtitle">{`${favorite.price} ₽ `}</div>
                        <div className="likeInfo__itemInfo-buttons">
                          <div
                              className="likeInfo__itemInfo-buttonBasket"
                              onClick={() => handleAddToBasketClick(favorite)}
                          >
                            <div className="likeInfo__itemInfo-icon icon-basket"></div>В корзину
                          </div>
                          <div
                              className="likeInfo__itemInfo-buttonTrash"
                              onClick={() => handleRemoveFavoritesClick(favorite.id ?? 0)}
                          >
                            <svg
