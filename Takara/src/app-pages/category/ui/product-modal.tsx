...
export const ProductModal: FC<ProductModalProps> = ({
  ...
  const [buttonIsFavoriteActive, setButtonIsFavoriteActive] = useState(false);
  const {
    data: favorites,
    isLoading,
    isFetching,
  ...
  useEffect(() => {
    setButtonIsFavoriteActive(isFavorite);
  }, [isOpen, isFavorite]);
  ...
  const handleAddToFavoritesClick = async () => {
    if (accessToken === null) {
      ...
      //todo fix types
      product?.id && await createFavorite({ productId: product?.id }).unwrap();
      setButtonIsFavoriteActive(!buttonIsFavoriteActive);
    }
  };

  const handleRemoveFavoritesClick = () => {
    product?.id && removeFavorite(product?.id);
    setButtonIsFavoriteActive(!buttonIsFavoriteActive);
  };

  //больше 50 штук в порции не должно ведь быть?)
  const renderPortion = (value: string | undefined | null) => {
    const firstArr = ['1', '21', '31', '41', '51', '61', '71', '81', '91'];
    const secondArr = ['2', '3', '4', '22', '23', '24', '32', '33'];

    if (value) {
      if (firstArr.includes(value)) {
        return `${value} штука`;
      } else if (secondArr.includes(value)) {
        return `${value} штук`;
      } else {
        return `${value} штуки`;
      }
  ...
  return (
    ...
    >
      <div className="modalFood__grid">
        <Image src={product?.photoPath ?? '/img/defaultCard.png'} alt="" width={770} height={600} />
