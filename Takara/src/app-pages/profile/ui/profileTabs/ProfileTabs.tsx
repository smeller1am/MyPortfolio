...
import { ProfileEntities } from '@/app-pages/profile/ui/profile';
import store, {persistor} from '@/app/store';
import { logout } from '@/app/store/authSlice';
import { useSetLogoutMutation } from '@/shared/clientApi';
...
export interface ProfileTabsProps {
  ...
  data?: GetUserResponseRestApiResponse
}

const ProfileTabs: FC<ProfileTabsProps> = ({ changeCategoryIndex, activeCategory, data }) => {
  const router = useRouter();
  const [logout] = useSetLogoutMutation();
  ...
  return (
    <div className="profileChoise">
      <div
        className={cn('choiseItem', {
          'choiseItem--active': activeCategory === ProfileEntities.Profile,
        })}
        onClick={() => changeCategoryIndex(ProfileEntities.Profile)}
      >
        <div className="choiseItem__icon icon-user"></div>
        <div className="choiseItem__nickname">{data?.payload?.user.firstName} {data?.payload?.user.lastName}</div>
        <div className="choiseItem__points">{data?.payload?.user.points} {declination(data?.payload?.user.points)}</div>
      </div>
      <div className="profileChoise__likeHistory">
        <div
          className={cn('choiseItem profileChoise__likeHistory-like', {
            'choiseItem--active': activeCategory === ProfileEntities.Favorite,
          })}
        ...
        <div
          className={cn('choiseItem profileChoise__likeHistory-like', {
            'choiseItem--active': activeCategory === ProfileEntities.History,
          })}
...
export const ProfileTabsWithProvider: FC<ProfileTabsProps> = props => (
